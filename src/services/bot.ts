...
//     // --- Find and click the Care Central app in the "My Top Applications" section ---
//     let clicked = false;

//     // Try to find the Care Central app card by its unique structure
//     // Only use valid selectors (no :has or :has-text)
//     const careCentralSelector = [
//       // The most direct: anchor with title
//       'div.my-top-apps a[title="Care Central"]',
//       // Fallback: h3.card-title with text, then get its closest anchor
//       // (handled below in evaluate)
//     ];

//     let careCentralElement = null;
//     for (const selector of careCentralSelector) {
//       careCentralElement = await safeOperation(() => page.$(selector), null);
//       if (careCentralElement) break;
//     }

//     if (careCentralElement) {
//       // Try to click the anchor directly
//       await safeOperation(() => careCentralElement!.click(), null);
//       clicked = true;
//       await delay(4000);

//       // Check if we've navigated away from the dashboard
//       const currentUrl = page.url();
//       if (!currentUrl.includes("dashboard") || currentUrl.includes("care-central")) {
//         // Success
//       } else {
//         clicked = false;
//       }
...
//     // Fallback: try to find the Care Central card by traversing the DOM (no :has, no :has-text)
//     if (!clicked) {
//       clicked = await safeOperation(
//         () =>
//           page.evaluate(() => {
//             // Find all app cards in the my-top-apps container
//             const container = document.querySelector('.my-top-apps');
//             if (!container) return false;
//             const cards = Array.from(container.querySelectorAll('.card'));
//             for (const card of cards) {
//               // Look for a h3.card-title with text "Care Central"
//               const h3s = card.querySelectorAll('h3.card-title');
//               for (const h3 of h3s) {
//                 if (h3 && h3.textContent && h3.textContent.trim() === "Care Central") {
//                   // Find the anchor inside the card
//                   const anchor = card.querySelector('a[title="Care Central"]') || card.querySelector('a');
//                   if (anchor) {
...
//     // Fallback: try to click by coordinates (last resort)
//     if (!clicked) {
//       // Try to find the bounding box of the Care Central card and click its center
//       const box = await safeOperation(
//         () =>
//           page.evaluate(() => {
//             const container = document.querySelector('.my-top-apps');
...
//         await safeOperation(() => page.mouse.click(box.x, box.y), null);
//         clicked = true;
//         await delay(4000);
//       }
//     }
...
//     if (!clicked) {
//       throw new Error("Failed to click on Care Central after trying all DOM-based approaches");
//     }
...
//     // Strategy 1: Direct anchor selector with title
//     if (!clicked) {
//       console.log("Trying Strategy 1: Direct anchor with title selector...")
//       const careCentralAnchor = await safeOperation(() => page.$('a[title="Care Central"]'), null)
//       if (careCentralAnchor) {
//         try {
//           await safeOperation(() => careCentralAnchor.click(), null)
//           clicked = true
//           console.log("Successfully clicked Care Central using direct anchor selector")
//           await delay(4000)
//         } catch (error) {
//           console.log("Direct anchor click failed, trying next strategy")
//         }
//       }
//     }

//     // Strategy 2: Find by href containing care-central
//     if (!clicked) {
//       console.log("Trying Strategy 2: Anchor with care-central in href...")
//       const careCentralByHref = await safeOperation(() => page.$('a[href*="care-central"]'), null)
//       if (careCentralByHref) {
//         try {
//           await safeOperation(() => careCentralByHref.click(), null)
//           clicked = true
//           console.log("Successfully clicked Care Central using href selector")
//           await delay(4000)
//         } catch (error) {
//           console.log("Href-based click failed, trying next strategy")
//         }
//       }
//     }

//     // Strategy 3: Find by card structure and text content
//     if (!clicked) {
//       console.log("Trying Strategy 3: Card structure with text matching...")
//       clicked = await safeOperation(
//         () =>
//           page.evaluate(() => {
//             // Find all cards in the my-top-apps container
//             const container = document.querySelector(".my-top-apps") || document.querySelector('[class*="my-top-apps"]')
//             if (!container) {
//               console.log("No my-top-apps container found")
//               return false
//             }
...
//             for (const card of cards) {
//               // Look for h3 with "Care Central" text
//               const titleElements = card.querySelectorAll('h3, .card-title, [class*="card-title"]')
//               for (const titleEl of titleElements) {
//                 if (titleEl && titleEl.textContent && titleEl.textContent.trim() === "Care Central") {
//                   console.log("Found Care Central card by title text")
//                   // Find any clickable anchor in this card
//                   const anchors = card.querySelectorAll("a")
//                   for (const anchor of anchors) {
//                     if (anchor.href && (anchor.href.includes("care-central") || anchor.title === "Care Central")) {
//                       console.log("Clicking Care Central anchor")
//                       ;(anchor as HTMLElement).click()
//                       return true
//                     }
//                   }
//                   // If no specific anchor found, click the first anchor in the card
//                   const firstAnchor = card.querySelector("a")
//                   if (firstAnchor) {
//                     console.log("Clicking first anchor in Care Central card")
//                     ;(firstAnchor as HTMLElement).click()
//                     return true
//                   }
//                 }
//               }
...
//       )
//       if (clicked) {
//         console.log("Successfully clicked Care Central using card structure")
//         await delay(4000)
//       }
//     }

//     // Strategy 4: More aggressive text search across all elements
//     if (!clicked) {
//       console.log("Trying Strategy 4: Aggressive text search...")
//       clicked = await safeOperation(
//         () =>
//           page.evaluate(() => {
//             // Find all elements containing "Care Central" text
//             const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null)

//             let node
//             const careCentralElements = []

//             while ((node = walker.nextNode())) {
//               if (node.textContent && node.textContent.trim() === "Care Central") {
//                 careCentralElements.push(node.parentElement)
//               }
//             }
...
//                 const anchor = current.querySelector("a") || (current.tagName === "A" ? current : null)
//                 if (anchor) {
//                   console.log("Found clickable ancestor for Care Central text")
//                   ;(anchor as HTMLElement).click()
//                   return true
//                 }
...
//       )
//       if (clicked) {
//         console.log("Successfully clicked Care Central using aggressive text search")
//         await delay(4000)
//       }
//     }

//     // Strategy 5: Click by coordinates as last resort
//     if (!clicked) {
//       console.log("Trying Strategy 5: Click by coordinates...")
//       const box = await safeOperation(
//         () =>
//           page.evaluate(() => {
//             // Find Care Central card and get its bounding box
//             const container = document.querySelector(".my-top-apps") || document.querySelector('[class*="my-top-apps"]')
//             if (!container) return null
...
//           await safeOperation(() => page.mouse.click(box.x, box.y), null)
//           clicked = true
//           console.log("Successfully clicked Care Central using coordinates")
//           await delay(4000)
//         } catch (error) {
...
//       const hasNavigated =
//         currentUrl.includes("care-central") || currentUrl.includes("loadApp") || !currentUrl.includes("apps/home")

//       if (!hasNavigated) {
//         console.log("Click succeeded but navigation may not have completed, checking for iframe...")
//         // Sometimes the page loads in an iframe, check for that
//         const iframe = await safeOperation(() => page.$("#newBodyFrame"), null)
//         if (!iframe) {
//           clicked = false
//           console.log("No iframe found, considering click failed")
//         }
//       }
//     }
...
//     console.log("Tax ID is automatically selected, skipping to provider selection...")
...
//     try {
//       // Wait for the provider input field to be present
//       await safeOperation(() => newBodyFrame.waitForSelector("#selectedProvider", { timeout: 10000 }), null)
//       console.log("Found provider input field")

//       // Click on the provider input field to focus it
//       await safeOperation(() => newBodyFrame.click("#selectedProvider"), null)
//       await delay(500)

//       // Click on the dropdown arrow button to open the dropdown
//       const dropdownButton = await safeOperation(() => newBodyFrame.$('button[aria-label="Open"][title="Open"]'), null)
//       if (dropdownButton) {
//         await safeOperation(() => dropdownButton.click(), null)
//         console.log("Clicked dropdown arrow button")
//         await delay(1500)
//       } else {
//         // Try alternative selector for the dropdown button
//         await safeOperation(() => newBodyFrame.click('div[name="selectedProvider"] button[role="button"]'), null)
//         console.log("Clicked alternative dropdown button")
//         await delay(1500)
//       }

//       // Wait for dropdown options to appear and select Harmony Health
...
//         try {
//           console.log(`Looking for options with selector: ${selector}`)

//           // Wait for options to appear
//           await safeOperation(() => newBodyFrame.waitForSelector(selector, { visible: true, timeout: 5000 }), null)

//           // Get all options and find Harmony Health
//           const selected = await safeOperation(
//             () =>
//               newBodyFrame.evaluate((sel) => {
//                 const options = Array.from(document.querySelectorAll(sel))
...
//                   if (text.toLowerCase().includes("harmony health")) {
//                     console.log(`Found Harmony Health option: ${text}`)
//                     ;(option as HTMLElement).click()
//                     return true
//                   }
//                 }

//                 // If Harmony Health not found, try first non-empty option
//                 if (options.length > 0) {
//                   const firstOption = options[0]
//                   const firstText = firstOption.textContent || ""
//                   if (firstText.trim() && !firstText.toLowerCase().includes("select")) {
//                     console.log(`Selecting first available option: ${firstText}`)
...
//       // If still not selected, try typing approach
//       if (!providerSelected) {
//         console.log("Trying typing approach for provider selection...")

//         // Clear the input and type "Harmony Health"
//         await safeOperation(() => newBodyFrame.click("#selectedProvider"), null)
//         await delay(300)
...
//         // Type "Harmony Health"
//         await safeOperation(() => newBodyFrame.type("#selectedProvider", "Harmony Health"), null)
//         await delay(1000)

//         // Look for autocomplete suggestions
//         const suggestionSelected = await safeOperation(
//           () =>
//             newBodyFrame.evaluate(() => {
//               const suggestions = Array.from(document.querySelectorAll('li[role="option"], .MuiAutocomplete-option'))
//               console.log(`Found ${suggestions.length} autocomplete suggestions`)

//               for (const suggestion of suggestions) {
//                 const text = suggestion.textContent || ""
//                 if (text.toLowerCase().includes("harmony health")) {
//                   console.log(`Clicking autocomplete suggestion: ${text}`)
//                   ;(suggestion as HTMLElement).click()
//                   return true
//                 }
//               }

//               // If no exact match, press Enter to accept typed value
//               const input = document.querySelector("#selectedProvider") as HTMLInputElement
//               if (input && input.value.toLowerCase().includes("harmony")) {
//                 console.log("Pressing Enter to accept typed value")
...
//           providerSelected = true
//           console.log("Successfully selected provider via typing approach")
//         }
//       }
//     } catch (error) {
//       console.log(`Provider selection failed: ${error instanceof Error ? error.message : String(error)}`)
//     }

//     if (!providerSelected) {
//       throw new Error(
//         "Failed to select provider from dropdown after multiple attempts. Please check if 'Harmony Health' is available in the provider dropdown.",
//       )
//     }
...
//     const nextButtonSelectors = [
//       "#landing-next", // Specific ID from HTML
//       "button[type='submit']",
...
//       "button[value='Next']",
//       "input[value='Next']",
//       "button.btn:last-of-type", // Last button (likely to be Next)
//     ]
...
//         // Material-UI tab selectors based on the HTML structure provided
//         const incomingTabSelectors = [
//           "#mui-p-80862-T-incoming", // Specific ID from HTML
//           'button[id*="T-incoming"]', // Any button with ID containing "T-incoming"
//           'button[aria-controls*="P-incoming"]', // Button controlling incoming panel
//           'button[role="tab"][aria-selected="true"]', // Currently selected tab (likely incoming)
//           'button[role="tab"]:first-child', // First tab (usually incoming)
//           ".MuiTab-root:first-child", // First Material-UI tab
//           '.MuiButtonBase-root[role="tab"]:first-child', // First tab button
//         ]
...
//             for (const element of elements) {
//               try {
//                 // Check if this element is visible and is the incoming tab
//                 const isIncomingTab = await safeOperation(
//                   () =>
//                     element.evaluate((el) => {
//                       const text = el.textContent?.trim() || ""
//                       const id = el.id || ""
...
//                       // Check if it's the incoming tab by text, ID, or aria-controls
//                       return (
//                         text.toLowerCase().includes("incoming") ||
//                         id.includes("incoming") ||
//                         ariaControls.includes("incoming") ||
//                         el.getAttribute("aria-selected") === "true" // Already selected incoming tab
//                       )
//                     }),
//                   false,
//                 )
...
//                   await safeOperation(() => element.click(), null)
//                   tabClicked = true
//                   console.log(`Successfully clicked on 'incoming' tab with selector: ${selector}`)
//                   break
//                 }
//               } catch (elementError) {
//                 // Continue to next element
...
//         // If we couldn't find the tab by Material-UI selectors, try generic approaches
//         if (!tabClicked) {
//           try {
//             // Try to find any tab elements and click the first one (likely incoming)
//             const tabElements = await safeOperation(
//               () => frame.$$('button[role="tab"], .MuiTab-root, .nav-tabs .nav-item, .nav-tabs .nav-link'),
//               [],
//             )
//             if (tabElements.length > 0) {
//               await safeOperation(() => tabElements[0].click(), null)
//               tabClicked = true
//               console.log("Clicked on first tab (assuming it's 'incoming')")
//             }
//           } catch (error) {
//             console.log("Could not find any tab elements")
//           }
//         }

//         // Final fallback: try to find by text content using evaluate
//         if (!tabClicked) {
//           try {
//             const foundAndClicked = await safeOperation(
//               () =>
//                 frame.evaluate(() => {
//                   // Look for any element containing "Incoming" text
//                   const allElements = Array.from(document.querySelectorAll("*"))
//                   const incomingElement = allElements.find((el) => {
//                     const text = el.textContent?.trim().toLowerCase() || ""
//                     const isClickable =
//                       el.tagName === "BUTTON" || el.tagName === "A" || el.getAttribute("role") === "tab"
//                     return text === "incoming" && isClickable
//                   })
...
//             if (foundAndClicked) {
//               tabClicked = true
//               console.log("Successfully clicked incoming tab using text search fallback")
//             }
//           } catch (error) {
//             console.log("Text search fallback failed")
//           }
//         }

//         if (!tabClicked) {
//           console.log("Could not find and click 'incoming' tab with any method")
//           // Note: This is not necessarily a critical error since API-based checking works
//           console.log("Continuing with API-based referral checking...")
//         }
...
//     // Now extract member information from the referrals page
//     const initialMembers = await extractMembersFromFrame(currentFrame)
//     console.log(`Initial extraction found ${initialMembers.length} members`)

//     // Store these as current members for monitoring
...
//           console.log("[v0] Starting member extraction from Material-UI tables")

//           // Target the specific Material-UI table containers for referrals
//           const tableContainers = document.querySelectorAll("#tableContent-tab-container")
//           console.log(`[v0] Found ${tableContainers.length} table containers`)

//           tableContainers.forEach((container, containerIndex) => {
//             try {
//               // Look for the referral info row within each container
//               const referralRow = container.querySelector("tr#incoming-referrral-info")
//               if (!referralRow) {
//                 console.log(`[v0] No referral row found in container ${containerIndex + 1}`)
//                 return
//               }

//               console.log(`[v0] Processing referral row in container ${containerIndex + 1}`)

//               // Extract member name from the specific cell
//               const memberNameCell = referralRow.querySelector("td#memName")
//               const memberName = memberNameCell?.textContent?.trim() || "Unknown"

//               // Extract service name
//               const serviceCell = referralRow.querySelector("td#serviceCol")
//               const serviceName = serviceCell?.textContent?.trim() || ""

//               // Extract county
//               const countyCell = referralRow.querySelector("td#countyCol")
//               const county = countyCell?.textContent?.trim() || ""

//               // Extract region
//               const regionCell = referralRow.querySelector("td#regionCol")
//               const region = regionCell?.textContent?.trim() || ""

//               // Extract program
//               const programCell = referralRow.querySelector("td#programCol")
//               const program = programCell?.textContent?.trim() || ""

//               // Extract dates
//               const datesCell = referralRow.querySelector("td#datesCol")
//               let requestDate = ""
//               if (datesCell) {
//                 const preferredDateDiv = datesCell.querySelector("#preferred-date")
//                 if (preferredDateDiv) {
//                   const dateMatch = preferredDateDiv.textContent?.match(/\d{2}\/\d{2}\/\d{4}/)
//                   if (dateMatch) {
//                     requestDate = dateMatch[0]
//                   }
//                 }
//               }

//               // Extract status
//               const statusCell = referralRow.querySelector("td#statusCol")
//               let status = ""
//               if (statusCell) {
//                 const statusChip = statusCell.querySelector(".MuiChip-label")
//                 status = statusChip?.textContent?.trim() || ""
//               }

//               // Extract referral ID from the more details section
//               let memberID = `ref-${Date.now()}-${containerIndex}`
//               const moreDetailSection = container.querySelector("#more-detail-section")
//               if (moreDetailSection) {
//                 const refNumElement = moreDetailSection.querySelector("#moreDetail-refNum")
//                 if (refNumElement?.textContent?.trim()) {
//                   memberID = refNumElement.textContent.trim()
//                 }
//               }
...
//                   county,
//                   requestDate,
//                   additionalInfo: `Region: ${region}, Program: ${program}`,
//                 }

//                 console.log(`[v0] Successfully extracted member: ${JSON.stringify(memberData)}`)
//                 results.push(memberData)
//               } else {
//                 console.log(`[v0] Skipping container ${containerIndex + 1} - no valid member name found`)
//               }
//             } catch (err) {
//               console.log(`[v0] Error processing container ${containerIndex + 1}:`, err)
//             }
//           })

//           console.log(`[v0] Final extraction result: ${results.length} members found`)
//           return results
//         }),
//       [],
//     )
//   } catch (error) {
//     console.log("[v0] Error in extractMembersFromFrame:", error)
//     return []
//   }
// }
...
// // Check if browser needs to be restarted periodically to prevent memory leaks
// async function checkBrowserHealth(): Promise<void> {
//   const timeSinceLastRestart = Date.now() - lastRestartTime.getTime()
...
//     // Stop monitoring interval but don't change isMonitoring flag yet
//     if (monitoringInterval) {
//       clearInterval(monitoringInterval)
//       monitoringInterval = null
//     }
...
//     // Restore monitoring if it was active
//     if (wasMonitoring) {
//       console.log("Restoring monitoring after browser restart...")
//       await loginToAvaility()
//       // isMonitoring will be set back to true in loginToAvaility -> startContinuousMonitoring
//     } else {
//       isMonitoring = false
//     }
...
//         // Stop the current monitoring interval but keep isMonitoring true during recovery
//         if (monitoringInterval) {
//           clearInterval(monitoringInterval)
//           monitoringInterval = null
//         }
...
//         // Try to re-login and restart monitoring
//         try {
//           console.log("Attempting to recover and restart monitoring...")
//           isLoggedIn = false
//           currentFrame = null

//           await loginToAvaility()

//           console.log("Recovery successful, monitoring should be active again")
//         } catch (loginError) {
//           console.error("Failed to recover after frame detachment:", loginError)
...
//           setTimeout(async () => {
//             try {
//               console.log("Attempting full restart after recovery failure...")
//               await closeBrowser()
//               await setupBot()
//               await loginToAvaility()
//             } catch (restartError) {
...
//             }
//           }, 30000) // Wait 30 seconds before full restart
//         }
//       } else {
//         console.log("Non-critical error during monitoring, continuing...")
//       }
...
const BROWSER_RESTART_INTERVAL_MS = 3600000 // 1 hour - restart browser periodically to prevent memory leaks
const HOURLY_RESTART_INTERVAL_MS = 3000000 // 50 minutes in milliseconds
const MAX_NAVIGATION_FAILURES = 5 // Maximum consecutive navigation failures before switching to API-only mode
...
async function navigateToCareCentral(page: Page): Promise<void> {
  console.log("Navigating to Care Central via new Payer Spaces flow...")

  // Step 1: Click Payer Spaces dropdown and select WellPoint
  console.log("Looking for Payer Spaces dropdown button...")

  let wellPointClicked = false

  const payerSpacesSelectors = [
    'button[aria-label="Payer Spaces"]',
    'button[aria-haspopup="true"]:has-text("Payer Spaces")',
    '.NavDropdown__trigger:has-text("Payer Spaces")',
    'button:has-text("Payer Spaces")',
    '[data-testid*="payer-spaces"]',
    '[id*="payer-spaces"]',
  ]

  for (const selector of payerSpacesSelectors) {
    try {
      const element = await safeOperation(() => page.$(selector), null)
      if (element) {
        console.log(`Trying Payer Spaces dropdown selector: ${selector}`)
        await safeOperation(() => element.click(), null)
        console.log(`Successfully clicked Payer Spaces dropdown with selector: ${selector}`)

        // Wait for dropdown to fully open
        await delay(3000)

        console.log("Looking for filter input inside Payer Spaces dropdown...")

        // Try to find and use the filter within the dropdown
        const filterUsed = await safeOperation(
          () =>
            page.evaluate(() => {
              // Find the dropdown menu container first
              const dropdownContainers = Array.from(
                document.querySelectorAll('[role="menu"], .dropdown-menu, .NavDropdown__menu, [class*="dropdown"]'),
              )

              console.log(`[v0] Found ${dropdownContainers.length} dropdown containers`)

              for (const container of dropdownContainers) {
                // Look for filter/search input INSIDE this dropdown
                const filterInput = container.querySelector(
                  'input[placeholder*="Filter"], input[placeholder*="filter"], input[type="text"]',
                ) as HTMLInputElement

                if (filterInput) {
                  console.log(`[v0] Found filter input inside dropdown`)
                  filterInput.focus()
                  filterInput.value = "wellpoint"
        ...
        if (filterUsed) {
          console.log("Successfully typed 'wellpoint' in dropdown filter")
          await delay(2000) // Wait for filter results
        } else {
          console.log("No filter found in dropdown, will scroll to find WellPoint")
        }

        console.log("Looking for WellPoint in Payer Spaces dropdown menu...")

        const wellPointFound = await safeOperation(
          () =>
            page.evaluate(() => {
              // First, try to find elements specifically under "PAYER SPACE" section
              const allElements = Array.from(document.querySelectorAll("*"))

              // Look for the PAYER SPACE section header
              const payerSpaceSection = allElements.find((el) => el.textContent?.trim().toUpperCase() === "PAYER SPACE")

              console.log(`[v0] Found PAYER SPACE section: ${!!payerSpaceSection}`)

              let wellPointElements: Element[] = []

              if (payerSpaceSection) {
                // Get the parent container of the PAYER SPACE section
                let sectionContainer = payerSpaceSection.parentElement
                while (sectionContainer && !sectionContainer.querySelector('a, button, [role="menuitem"]')) {
                  sectionContainer = sectionContainer.parentElement
                }

                if (sectionContainer) {
                  // Look for WellPoint within this section
                  const sectionElements = Array.from(sectionContainer.querySelectorAll("*"))
                  wellPointElements = sectionElements.filter((el) => {
                    const text = el.textContent?.trim().toLowerCase() || ""
                    return (
                      text.includes("wellpoint") &&
                      !text.includes("search availity") && // Exclude help links
                      !text.includes("forms") && // Exclude forms
                      (el.tagName === "A" ||
                        el.tagName === "BUTTON" ||
                  ...
                  console.log(`[v0] Found ${wellPointElements.length} WellPoint elements in PAYER SPACE section`)
                }
              }

              // Fallback: look for WellPoint in dropdown, but exclude TOP HIT section
              if (wellPointElements.length === 0) {
                console.log(`[v0] Fallback: searching entire dropdown for WellPoint`)
                wellPointElements = allElements.filter((el) => {
                  const text = el.textContent?.trim().toLowerCase() || ""
                  ...
                  return (
                    text.includes("wellpoint") &&
                    !text.includes("search availity") && // Exclude help links
                    !text.includes("forms") && // Exclude forms from TOP HIT
                    !parentText.includes("top hit") && // Exclude TOP HIT section
                    (el.tagName === "A" ||
                      el.tagName === "BUTTON" ||
                      el.getAttribute("role") === "menuitem" ||
                      el.classList.contains("dropdown-item"))
                ...
                console.log(`[v0] Found ${wellPointElements.length} WellPoint elements (excluding TOP HIT)`)
              }

              // Click the first visible WellPoint element
              for (const element of wellPointElements) {
                const rect = element.getBoundingClientRect()
                if (rect.width > 0 && rect.height > 0) {
                  console.log(`[v0] Clicking WellPoint element: ${element.textContent?.trim()}`)
        ...
        if (wellPointFound) {
          console.log("Successfully clicked WellPoint from Payer Spaces dropdown")
          wellPointClicked = true
          await delay(5000) // Wait for WellPoint page to load
          break
        } else {
          console.log("WellPoint not found in dropdown, trying next selector...")
        }
      }
    } catch (error) {
  ...
  if (!wellPointClicked) {
    throw new Error("Failed to find Payer Spaces dropdown and select WellPoint")
  }

  console.log("Looking for Care Central on WellPoint page...")
  await delay(8000) // Wait for page to fully load

  console.log("[v0] Debugging WellPoint page content...")
  console.log("[v0] Current URL:", page.url())
  console.log("[v0] Page title:", await page.title())

  // FIRST: Find the iframe that contains the WellPoint content
  console.log("[v0] Looking for iframes on the page...")
  await delay(3000) // Wait for iframes to load

  let careCentralClicked = false
  let targetFrame: Frame | null = null

  // Search for Care Central in each iframe
  for (const frame of page.frames()) {
    try {
      const frameName = frame.name()
      const frameUrl = frame.url()
      console.log(`[v0] Checking frame: name="${frameName}", url="${frameUrl}"`)

      // Skip the main frame
      if (frame === page.mainFrame()) {
        console.log("[v0] Skipping main frame")
        continue
      }
      ...
      // Strategy 1: Look for element with id="application-link-jHVW5aqCpFrfevdeIuDVx"
      console.log(`[v0] Strategy 1: Looking for Care Central by specific ID in frame "${frameName}"...`)
      const careCentralById = await frame.$("#application-link-jHVW5aqCpFrfevdeIuDVx")
      if (careCentralById) {
        console.log(`[v0] Found Care Central card by ID in frame "${frameName}"`)
        // Look for the link inside
        const link = await frame.$("#app-title-jHVW5aqCpFrfevdeIuDVx")
        if (link) {
          console.log("[v0] Found Care Central link by ID, clicking...")
          await link.click()
          careCentralClicked = true
          targetFrame = frame
          break
        }
      }

      // Strategy 2: Look for element with title="Care Central"
      console.log(`[v0] Strategy 2: Looking for Care Central by title attribute in frame "${frameName}"...`)
      const careCentralByTitle = await frame.$('[title="Care Central"]')
      if (careCentralByTitle) {
        console.log(`[v0] Found Care Central card by title in frame "${frameName}"`)
        // Look for the link inside
        const link = await careCentralByTitle.$("a.MuiLink-root")
        if (link) {
          console.log("[v0] Found Care Central link inside card, clicking...")
          await link.click()
          careCentralClicked = true
          targetFrame = frame
          break
        } else {
          console.log("[v0] No link found, clicking card directly...")
          await careCentralByTitle.click()
          careCentralClicked = true
          targetFrame = frame
          break
        }
      ...
      // Strategy 3: Look for any link with text "Care Central"
      console.log(`[v0] Strategy 3: Looking for Care Central link by text in frame "${frameName}"...`)
      const careCentralLinks = await frame.$$("a")
      for (const link of careCentralLinks) {
        const text = await frame.evaluate((el) => el.textContent, link)
        if (text && text.includes("Care Central")) {
          console.log(`[v0] Found Care Central link with text "${text}" in frame "${frameName}", clicking...`)
          await link.click()
          careCentralClicked = true
          targetFrame = frame
          break
      ...
      if (careCentralClicked) {
        break
      }
    } catch (frameError) {
      console.log(`[v0] Error checking frame: ${(frameError as Error).message}`)
      continue
    }
  }

  // If not found in any iframe, try the main page as fallback
  if (!careCentralClicked) {
    console.log("[v0] Care Central not found in any iframe, trying main page...")

    // Try the main page strategies
    const mainPageResult = await safeOperation(async () => {
      // Strategy 1: By title attribute
      const cardByTitle = await page.$('[title="Care Central"]')
      if (cardByTitle) {
        console.log("[v0] Found Care Central by title on main page")
        const link = await cardByTitle.$("a.MuiLink-root")
        if (link) {
          await link.click()
          return true
        } else {
          await cardByTitle.click()
          return true
        }
      }

      // Strategy 2: By ID
      const cardById = await page.$("#application-link-jHVW5aqCpFrfevdeIuDVx")
      if (cardById) {
        console.log("[v0] Found Care Central by ID on main page")
        const link = await page.$("#app-title-jHVW5aqCpFrfevdeIuDVx")
        if (link) {
          await link.click()
          return true
        } else {
    ...
    careCentralClicked = mainPageResult
  }

  if (!careCentralClicked) {
    throw new Error("Failed to find and click Care Central in any frame or main page")
  }

  console.log("[v0] Successfully clicked Care Central!")
  await delay(5000)

  // Wait for the iframe to load (rest of the existing logic remains the same)
  await safeOperation(() => page.waitForSelector("#newBodyFrame", { timeout: 40000 }), null)
  ...
  currentFrame = newBodyFrame

  // Wait for the form to load in the iframe
  await safeOperation(() => currentFrame!.waitForSelector("form", { timeout: 40000 }), null)

  // Wait for the organization dropdown to be present in the iframe
  await safeOperation(() => currentFrame!.waitForSelector("#organizations", { timeout: 40000 }), null)

  // Click on the organization dropdown
  await safeOperation(() => currentFrame!.click("#organizations"), null)
  await delay(1000)
  ...
  for (const selector of dropdownSelectors) {
    try {
      await safeOperation(() => currentFrame!.waitForSelector(selector, { visible: true, timeout: 10000 }), null)
      console.log(`Found dropdown options with selector: ${selector}`)
      dropdownOpened = true
  ...
  try {
    // Wait for the provider input field to be present
    await safeOperation(() => currentFrame!.waitForSelector("#selectedProvider", { timeout: 10000 }), null)
    console.log("Found provider input field")

    // Click on the provider input field to focus it
    await safeOperation(() => currentFrame!.click("#selectedProvider"), null)
    await delay(500)

    // Click on the dropdown arrow button to open the dropdown
    const dropdownButton = await safeOperation(() => currentFrame!.$('button[aria-label="Open"][title="Open"]'), null)
    if (dropdownButton) {
      await safeOperation(() => dropdownButton.click(), null)
      console.log("Clicked dropdown arrow button")
      await delay(1500)
      ...
      // Try alternative selector for the dropdown button
      await safeOperation(() => currentFrame!.click('div[name="selectedProvider"] button[role="button"]'), null)
      console.log("Clicked alternative dropdown button")
      await delay(1500)
    }
    ...
    for (const selector of optionSelectors) {
      try {
        ...
        // Wait for options to appear
        await safeOperation(() => currentFrame!.waitForSelector(selector, { visible: true, timeout: 5000 }), null)
    ...
    if (!providerSelected) {
      ...
      // Type "Harmony Health"
      await safeOperation(() => currentFrame!.type("#selectedProvider", "Harmony Health"), null)
      await delay(1000)
  ...
  for (const selector of nextButtonSelectors) {
    try {
      await safeOperation(() => currentFrame!.click(selector), null)
      console.log(`Successfully clicked Next button with selector: ${selector}`)
      nextButtonClicked = true
      break
    } catch (error) {
      ...
      () =>
        currentFrame!.evaluate(() => {
          ...
          const nextButton = buttons.find((btn) => {
            ...
            const value = (btn as HTMLInputElement).value?.toLowerCase() || ""
            return (
              text.includes("next") || text.includes("continue") || value.includes("next") || value.includes("continue")
            )
          })
          if (nextButton) {
            console.log("Found Next button by text content, clicking it")
  ...
  // Now we need to click on the Referrals button inside the iframe
  const refreshedFrames = page.frames()
  const refreshedNewBodyFrame = refreshedFrames.find((frame) => frame.name() === "newBody")
  if (!refreshedNewBodyFrame) {
    throw new Error("Could not find newBody iframe after navigation")
  }
  currentFrame = refreshedNewBodyFrame
      ...
      () =>
        frame.evaluate(() => {
          ...
          tableContainers.forEach((container, containerIndex) => {
            try {
              // Look for the referral info row within each container
              const referralRow = container.querySelector("tr#incoming-referral-info")
              if (!referralRow) {
                console.log(`[v0] No referral row found in container ${containerIndex + 1}`)
                return
              }
...
async function processNewMembers(members: MemberData[]): Promise<void> {
  ...
  try {
    ...
    // Send SMS for each new member
    for (const member of unnotifiedMembers) {
      await sendSMS(`New referral: ${member.memberName} (${member.memberID}). Check dashboard for details.`)
    }
    console.log("SMS notifications sent for new members")
  } catch (error) {
    console.error("Error processing new members:", error)
