...
import {
  ...
  sendStillAliveNotification,
  switchToApiOnlyMode,
  isApiOnlyMode,
  restartBot, // Import the restartBot function
} from "./services/bot"
import { scheduleStatusEmails } from "./scheduled-status"
import { connectToDatabase, isDatabaseConnected } from "./services/database"
import { StatusLog } from "./models/status-log"
...
async function startBot(): Promise<void> {
  ...
  try {
    ...
    // Only attempt login if not in API-only mode
    let loginSuccess = true
    if (!isApiOnlyMode) {
      loginSuccess = await loginToAvaility() // Attempt to log in
    }
...
// Route to switch to API-only mode
app.post("/api-only-mode", async (req, res) => {
  try {
    addLog("Manually switching to API-only mode...")
    await switchToApiOnlyMode()
    addLog("Switched to API-only mode successfully")
    res.status(200).json({ message: "Switched to API-only mode successfully!" })
  } catch (error) {
    addLog(`Error switching to API-only mode: ${error instanceof Error ? error.message : String(error)}`)
    res.status(500).json({
      message: "Failed to switch to API-only mode",
      error: error instanceof Error ? error.message : String(error),
    })
  }
})

// Route to manually trigger a restart
app.post("/restart-bot", async (req, res) => {
  try {
    addLog("Manually triggering bot restart...")
    await restartBot()
    addLog("Bot restart triggered successfully")
    res.status(200).json({ message: "Bot restart triggered successfully!" })
  } catch (error) {
    addLog(`Error triggering bot restart: ${error instanceof Error ? error.message : String(error)}`)
    res.status(500).json({
      message: "Failed to trigger bot restart",
      error: error instanceof Error ? error.message : String(error),
    })
  }
})
